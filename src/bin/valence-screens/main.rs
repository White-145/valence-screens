mod matrix_game_manager;
mod paint_game_manager;
mod rainbow_game_manager;
mod snake_game_manager;
...
use crate::matrix_game_manager::MatrixGameManager;
use crate::paint_game_manager::PaintGameManager;
use crate::rainbow_game_manager::RainbowGameManager;
use crate::snake_game_manager::SnakeGameManager;
use valence_screens::buffer::ScreenBuffer;
use valence_screens::game_manager::GameManager;
use valence_screens::{build_screen, input, Screen, ScreenPlugin};

const SPAWN_Y: i32 = 64;

pub fn main() {
    App::new()
        .add_plugins((DefaultPlugins, ScreenPlugin))
...
fn build(
    ...
    layer
        .chunk
        .set_block([0, SPAWN_Y, 0], BlockState::COBBLESTONE);
    ...
    for x in 0..HORIZONTAL_SIZE as i32 {
        layer
            .chunk
            .set_block(SCREEN_POS.add([x, 0, 0].into()), middle_bottom_block);
        for y in 0..VERTICAL_SIZE as i32 {
            layer
                .chunk
                .set_block(SCREEN_POS.add([x, 1 + y, 0].into()), screen_block);
        }
    }
    layer
        .chunk
        .set_block(SCREEN_POS.add([0, 0, 0].into()), edge_bottom_block);
    layer.chunk.set_block(
        SCREEN_POS.add([HORIZONTAL_SIZE as i32 - 1, 0, 0].into()),
        edge_bottom_block,
    );
...
fn init_clients(
    ...
    ) in &mut clients
    {
        ...
        *game_mode = GameMode::Adventure;
        ...
        // required for inputs
        input::init_client(&mut commands, entity, &mut screen, screen_id);
        inventory
            .as_mut()
            .set_slot(36, input::get_controller_item());
    }
}
...
fn chat(
    ...
    for event in chat_message_event.read() {
